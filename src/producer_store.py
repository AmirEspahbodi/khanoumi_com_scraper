...
async def _atomic_write(entries: list[dict[str, Any]]) -> None:
    ...
    # Retry loop for Windows transient file locks (WinError 5 or 32)
    max_retries = 5
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.warning(
                    "_atomic_write: Access denied on %s, retrying (%d/%d)...",
                    PRODUCER_JSON_PATH,
                    attempt + 1,
                    max_retries,
                )
                ...
                logger.error(
                    "_atomic_write: Failed to replace file after %d attempts.",
                    max_retries,
                )
                raise
